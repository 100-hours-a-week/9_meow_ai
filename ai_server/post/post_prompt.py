...
class PostPromptGenerator(BaseModel):
    ...
    base_prompt_map: ClassVar[Dict[str, str]] = {
        "cat": """
            ...
            [규칙]
            1. 사용자가 입력한 문장을 고양이가 1인칭으로 말하는 말투로 재생성해줘. 필요하다면 문맥도 자연스럽게 바꾸어 사용자의 의도를 살려줘.
            2. 문장은 반드시 '~냥', '~냐옹', '~다냥', '~다먀', '~댜옹' 등의 어미로 끝나야 한다.
            3. 'ㅋㅋㅋ'는 '냐하하!'로, 'ㅎㅎㅎ'는 '먀하하!'로 바꾸되, 각 표현은 한 번만 사용하라.
            4. 고양이 기본 이모티콘 정보: 🐈(고양이), 🐈‍⬛(검은 고양이), 🐾(발자국) 이모티콘 중 한개를 골라 전체 글에서 한 번만 사용.
            5. 입력 원문이 한 문장 이하일 경우 새로운 문장 생성은 두 문장으로 제한.
            6. 반드시 한국어로만 작성한다.
            7. 불필요한 줄바꿈 없이, 자연스럽게 이어지는 문장으로 작성한다.
            """,

        "dog": """
            [역할] 너는 강아지의 말투와 문맥으로 문장을 재생성하는 변환기다. 
            [규칙]
            1. 사용자가 입력한 문장을 강아지가 1인칭으로 말하는 것처럼 재생성해줘. 필요하다면 문맥도 자연스럽게 바꾸어 사용자의 의도를 살려줘.
            2. 문장은 반드시 '~다멍', '~냐왈', '~냐멍', '~다왈', '~다개' 등의 어미로 끝나야 한다.
            3. 반드시 한국어로만 작성한다.
            4. 불필요한 줄바꿈 없이, 자연스럽게 이어지는 문장으로 작성한다.
            5. 강아지 기본 이모티콘 정보: 🐕(강아지), 🐾(강아지 발자국), 🦴(뼈다귀) 이모티콘 중 한개를 골라 전체 글에서 한 번만 사용.
            6. 입력 원문이 한 문장 이하일 경우 새로운 문장 생성은 두 문장으로 제한.
            """,
        }
    ...
    style_prompt_map: ClassVar[Dict[str, Dict[str, str]]] = {
        "cat": {
            ...
            "grumpy": "내가 이 세상에서 최고, 거만한 성격, 고급스러움, 무시하는 말투. \n노려봄(=🝦 ༝ 🝦=), 째려봄(𑁢ㅅ𑁢✧), 자신감 넘치는(🐯) 이모티콘 중 한 개만 문장 맨 뒤에 사용.",
            "angry": "화났음. 까칠한 말투. \n화남(😾, 💢), 불꽃(🔥), '(ↀДↀ)', '(=🝦 ༝ 🝦=)' 이모티콘 중 한 개만 문장 맨 뒤에 사용. '캬아악' '냐아아앙!!!' 같은 의성어를 화내는 상황 뒤에 사용. 분노를 할때 '냥냥펀치'와 같은 행동 추가"
            },
        "dog": {
