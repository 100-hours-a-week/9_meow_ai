...
print(f"Debug - GOOGLE_API_KEYS: {single_key}")
print(f"Debug - GOOGLE_API_KEYS_list: {multiple_keys}")

if multiple_keys:
    try:
        # JSON 형식의 문자열을 파싱
        api_keys = json.loads(multiple_keys)
        if not isinstance(api_keys, list):
            print("Warning: GOOGLE_API_KEYS_list must be a list")
            api_keys = []
        else:
            print(f"Running in multi-key mode with {len(api_keys)} keys")
            print(f"Debug - Parsed keys: {api_keys}")
    except json.JSONDecodeError as e:
        print(f"Warning: Failed to parse GOOGLE_API_KEYS_list as JSON: {e}")
        api_keys = []
    except Exception as e:
        print(f"Warning: Unexpected error parsing GOOGLE_API_KEYS_list: {e}")
        api_keys = []
elif single_key:
    try:
        # 단일 키를 리스트로 변환
        ...
        print(f"Debug - Parsed key: {api_keys}")
    except Exception as e:
...
if not api_keys:
    ...
    print(f"Debug - Final api_keys list: {api_keys}")
